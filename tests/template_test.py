...
class TemplateTest(RequestsTest):
    def test(self):
        passed = 0
        failed = 0
        url = self.domain['protocol'] + self.domain['host'] + self.config['path']
        print("Template Injection Test for " + url)
        result_text = []
        result = 'PASS'
        for k, v in self.config['params'].items():
            for p in self.payloads:
                ...
                if self.config['method'] == 'GET':
                    ...
                    if res.status_code != 200:
                        failed += 1
                        matched = match_any(res.text, 'template') or match_any(res.text, 'generic')
                        if matched:
                            result = 'FAIL'
                            result_text.append('=> Payload ' + p + ' caused a template error in parameter ' + k)
                            sys.stderr.write('=> Payload ' + p + ' caused a template error in parameter ' + k + '\n')
                        else:
                            result = 'ERROR'
            ...
            self.report.add_test_result(url, self.config['method'], 'template', k, result, result_text)
